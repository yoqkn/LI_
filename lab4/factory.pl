...
satVariable( duration(T, H) ):- task(T), integer(H).
...
writeClauses(MaxHours):-
    ...
    taskInDuration(MaxHours),
    resourceUnitsAtMost(MaxHours),
    true,!.
...
taskInDuration(MaxHours):- 
	duration(T,D),
	LastSH is MaxHours-D,	
	between(1, LastSH, H),
	LastHour is H+D-1,
	between(H, LastHour, Hr),
	writeClause([-start(T,H), duration(T,Hr)]), 
	fail.
taskInDuration(_).	
...
resourceUnitsAtMost(MaxHours):-
	...
	findall(duration(T,H), usesResource(T,R), Lits),
	atMost(U, Lits),
	fail.
resourceUnitsAtMost(_).	
	
	
%eachTaskUsesResource(
