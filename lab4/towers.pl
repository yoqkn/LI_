...
%allWatched
%eachSignificantVillageOneTower
%%%%%%% =======================================================================================
...
village(a, 2,20,2).           % village(ident,row,col,size):  row,col of left upper corner, and size of the square*village(b, 3,13,2).
village(c, 7,21,2).
village(d, 8,13,2).
village(e,10,30,4).
village(f,11, 7,3).
...
noTowerOutsideVillage:- 
	position(I,J),
	\+posVillage(_,I,J), 
	findall(towerPos(I,J), position(I,J), L),
	exactly(0,L),
	fail.
noTowerOutsideVillage.
...
isTowerVillage:- 
	posVillage(V,I,J),
	writeClause([-towerPos(I,J), towerVillage(V)]), 
	fail.
isTowerVillage.	
atMostNTowers(MaxNTowers):- 
	findall(towerVillage(V), village(V), Lits), 
	atMost(MaxNTowers, Lits),
	fail.
atMostNTowers(_).	
...
costOfThisSolution(M,Cost):- findall(position(I,J),  member(towerPos(I,J), M), Lits), length(Lits, Cost),!.
