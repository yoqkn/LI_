...
%exactlyOneAgainstForLeague
%exactlyOneMatchForRound
%double
%noTriples

writeClauses(MaxCost):-
    eachTeamEachRoundExactlyOneMatch,
    eachPairExactlyOnePlay,
    isHomeorAway,
    isDouble,
    noTriples,
    noDoublesCertainRounds,
    eachRoundAtLeastOneMatch,
    teamNoHomeCertainRound,
%double
...
eachPairExactlyOnePlay:- difTeams(S,T),
...
noTriples:- 
	...
	R1 is R2-1, 
	team(S),
	writeClause([-double(S,R1),-double(S,R2)]), 
	fail.
noTriples.
...
eachRoundAtLeastOneMatch:-
	round(R),
	tvTeams(Ts),
	findall(match(S,T,R), (member(S,Ts), member(T,Ts), difTeams(S,T)), Lits),
	atLeast(1,Lits),
	fail.
eachRoundAtLeastOneMatch.
