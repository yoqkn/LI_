...
%%%%%%  2. Clause generation:
writeClauses:-
    eachGuestSitInOneTable,
    guestsMustSitTogether,
    notIncompatibleGuests,
    eachTableMinMaxPeople,
    true.
...
%%Pairs of guests who MUST sit together in the same table
guestsMustSitTogether:-
    mustSitTogether(G1,G2),
    table(T),
    writeClause([-guestTable(G1,T),
    guestTable(G2,T)]),
    fail.
guestsMustSitTogether.

%%Pairs of guests who CANNOT sit together in the same table
notIncompatibleGuests:-
    ...
    fail.
notIncompatibleGuests.
%%There are M available tables, each of which can fit between L and U people. ALL TABLES must be used.
eachTableMinMaxPeople:-
    table(T),
