...
allMembersFriends:- student(S1), student(S2), S1\=S2, \+friends(S1,S2), writeClause([-chosen(S1),-chosen(S2)]), fail.
allMembersFriends.

allDifferentTopic:- student(S1),student(S2),expertise(S1,E),expertise(S2,E), writeClause([-chosen(S1),-chosen(S2)]), fail.
allDifferentTopic.

exactlyKMembers:- findall(chosen(S), student(S),Lits), numMembersTeam(K), exactly(K,Lits),fail.
exactlyKMembers.
