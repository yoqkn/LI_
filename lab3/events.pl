...
satVariable( ed(E,D) ):-  event(E), day(D).   %% Event e is taked day D
satVariable( em(E,M) ):-  event(E), moderator(M).   %% Event e  moderator M
satVariable( md(M,D) ):-  moderator(M), day(D).   %% Moderator M day D

%%%%%%  2. Clause generation:

writeClauses:-  
    eachEventPossiblesDaysAndModerators,
    eachEventExactlyOneModerator, 
    eachEventExactlyOneDay,
    eachDayAtMostNEvents,
    eachModeratorAtMostKDays,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachEventPossiblesDaysAndModerators:- eventModerators(E,Ms), member(M,Ms), eventDays(E,Ds), member(D,Ds), writeClause([-ed(E,D),-em(E,M),md(M,D)]),fail.
eachEventPossiblesDaysAndModerators.

eachEventExactlyOneModerator:- event(E), eventModerators(E,Ms), findall(em(E,M), member(M,Ms),Lits), exactly(1,Lits),fail.
eachEventExactlyOneModerator.

eachEventExactlyOneDay:- event(E),  eventDays(E,Ds), findall(ed(E,D), member(D,Ds), Lits), exactly(1,Lits),fail.
eachEventExactlyOneDay.

eachDayAtMostNEvents:- day(D), maxEventsPerDay(N), findall(ed(E,D), event(E), Lits), atMost(N,Lits),fail.
eachDayAtMostNEvents.

eachModeratorAtMostKDays:- moderator(M), maxDaysPerModerator(K), findall(md(M,D),day(D),Lits), atMost(K,Lits),fail.
eachModeratorAtMostKDays.
%
%eachDayAtMostEvents
%eachModeratorAtMostDats
