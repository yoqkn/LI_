...
%eachBusExactlyOneTraject
%eachTrajectAtMost
%eachTrajectAtLeast
%eachBusAtMostKmsTwoConsecutivesDays
...
%%%%%% Some helpful definitions to make the code  maxDist(Maxcleaner:
...
writeClauses:-  
    ...
    eachTripAtMostOnePerDay,
    eachTripAtLeast,
    nextTrip,
    atMostKmsOfTwoConsecutivesDays,
    true,!.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

%Per cada bus en un dia concret només es fa un un viatje.
exactlyOneTripPerBusAndDay:- bus(B), day(D),findall(bdcc(B,D,C1,C2), trip(C1-C2), Lits), exactly(1,Lits),fail.
exactlyOneTripPerBusAndDay.

%Cada viatge en un dia concret només es realitza un sol cop.
eachTripAtMostOnePerDay:- day(D), trip(C1-C2),  findall(bdcc(B,D,C1,C2), bus(B), Lits), atMost(1,Lits),fail.
eachTripAtMostOnePerDay.

%Donat un traject i la seva freqüència F, s'ha de realitzar com a minim F trajectes.
eachTripAtLeast:- frequency(C1-C2,F), findall(bdcc(B,D,C1,C2), (bus(B),day(D)), Lits), atLeast(F,Lits),fail.
eachTripAtLeast.

%FI d'un és inici de l'altre
nextTrip:- trip(C1-C2),trip(C3-C4), C2\=C3, consecutiveDays(D1,D2), bus(B), writeClause([-bdcc(B,D1,C1,C2),-bdcc(B,D2,C3,C4)]), fail.
nextTrip.

%Dues dies consecutives com a molt K kms.
atMostKmsOfTwoConsecutivesDays:- trip(C1-C2),trip(C2-C3), consecutiveDays(D1,D2), bus(B), tooLongDist(C1-C2-C3), writeClause([-bdcc(B,D1,C1,C2),-bdcc(B,D2,C2,C3)]), fail.
atMostKmsOfTwoConsecutivesDays.
