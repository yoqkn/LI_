...
writeClauses:- 
      %atmostOneStationPerCity,
      atLeastOneStationPerRoad, 
      atMostNStation, 
      true,!.
...
%%
atmostOneStationPerCity:- city(C), findall(install(C), city(C), Lits), atMost(1,Lits), fail.
atmostOneStationPerCity.

atLeastOneStationPerRoad:- road(C1,C2), writeClause([(install(C1)),(install(C2))]),fail.
atLeastOneStationPerRoad.

atMostNStation:- maxStations(N), findall(install(C),city(C), Lits), atMost(N,Lits),fail.
atMostNStation.
%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

displaySol(M):- nl, member(install(C),M), write('Gas station '), write(C), nl, fail.
displaySol(_).
%displaySol(M):- member(install(C),M), write(C), nl, fail.
