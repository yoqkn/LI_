...
%En la posición P produeix Opció opt
...
eachCarOnePosition:- car(C), findall(carPos(C,P), position(P),Lits),exactly(1,Lits),fail.
eachCarOnePosition.

eachPositionOneCar:- position(P), findall(carPos(C,P),car(C),Lits),exactly(1,Lits),fail.
eachPositionOneCar.

relateCarPosOption:- car(C), position(P), carOption(C,Opt), writeClause([-carPos(C,P),posOpt(P,Opt)]), fail.
relateCarPosOption.

constraints:- option(Opt, Max, WindowSize), windowOfSize(WindowSize,WindowStart,WindowEnd),findall(posOpt(P,Opt), between(WindowStart,WindowEnd,P), Lits), atMost(Max,Lits),fail.
constraints.
