...
writeClauses:-   
    exactlyKChosen,
    differentsNeighborhood,
    atLeastOneRepresentative,
    true,!.                    % this way you can comment out ANY previous line of writeClauses
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.

exactlyKChosen:- findall(chosen(N), neighbor(N), Lits), numRepresentatives(K), exactly(K,Lits),fail.
exactlyKChosen.

differentsNeighborhood:- neighborNeighborhood(N1,NH), neighborNeighborhood(N2,NH), N1\=N2, writeClause([-chosen(N1),-chosen(N2)]),fail.
differentsNeighborhood.

atLeastOneRepresentative:- neighbor(N), neighborRepresentedBy(N,L), findall(chosen(R), member(R,L), Lits), atLeast(1,Lits),fail.
atLeastOneRepresentative.
