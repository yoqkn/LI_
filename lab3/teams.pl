...
incompatibleWorkers(W1,W2):- W1 \= W2, workerScore(W1,S1), workerScore(W2,S2), maxScore(X), Sum is S1+S2, X < Sum.  %% Complete this!
...
compatibleTeams:- worker(W1),worker(W2), incompatibleWorkers(W1,W2), team(T), writeClause([-wt(W1,T),-wt(W2,T)]),fail.
compatibleTeams.

eachWorkerExactlyOneTeam:- worker(W), findall(wt(W,T), team(T), Lit), exactly(1,Lit),fail.
eachWorkerExactlyOneTeam.

eachTeamAtLeast:- team(T), findall(wt(W,T), worker(W),Lit), minSize(N),atLeast(N,Lit),fail.
eachTeamAtLeast.

eachTeamAtMost:- team(T), findall(wt(W,T), worker(W),Lit), maxSize(N),atMost(N,Lit),fail.
eachTeamAtMost.
