...
eachTeamExactlyNStudents:- team(T), findall(st(S,T), student(S), Lits), teamSize(N), exactly(N,Lits),fail.
eachTeamExactlyNStudents.

atMostOneIncompatible:- incompatible(S1,S2), incompatible(S2,S3), team(T), writeClause([-st(S1,T),-st(S2,T), -st(S3,T)]), fail.
atMostOneIncompatible.
%%%%%%%%%%%%%%%%%%%%%%%
