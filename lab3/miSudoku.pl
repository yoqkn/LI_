...
satVariable( x(I,J,K) ):- row(I), col(J), val(K).	%///aquest es un varible si esta entre el rang

%%%%%%  2. Clause generation for the SAT solver:

writeClauses:- 		       % ///generar tots els clausules que considerem que necessitem
...
filledInputValues:- entrada(Sud), nth1(I,Sud,Row), nth1(J,Row,K), integer(K), writeClause([ x(I,J,K) ]), fail.	%///carreguem el sudoku(entrada(Sud)), nth1 ens diu quin es i-essim document(I-essim element de Sud es Row(una fila)), J-essim element de Row es K, K es un enter(fals, si es un "-" fara backtracking i agafara seguent columna), writeClause(escriure una clausula concret, generar clausules).
filledInputValues.

%% The Prolog predicate  findall(X, Cond, L)   means:   "L = { X | Cond } "
eachIJexactlyOneK:- row(I), col(J), findall( x(I,J,K), val(K), Lits ), exactly(1,Lits), fail. %donat x(I,J,K) tal que K es un variable entre 1 i 9 i ho posem a una llista Lits i nomes hi ha un d'aquest lista es cert.
eachIJexactlyOneK.
...
eachBlockEachKexactlyOnce:- blockID(Iid,Jid), 
        val(K), findall( x(I,J,K), squareOfBlock(Iid,Jid,I,J), Lits ), exactly(1,Lits), fail.   %primer establim un block i un valor per la K.
...
%displaySol(M):- nl, row(I), nl, line(I), col(J), space(J), member(x(I,J,K), M ), write(K), write(' '), fail.      %//mostrar la solucio de model M
%displaySol(_):- nl,nl.
