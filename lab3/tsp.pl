...
consecutivePositions(P1,P2):-  position(P1), position(P2), P2 is P1+1.
consecutiveCities(I1,I2):- city(I1), city(I2), I1\= I2, adjacency(I1,L1), member(I2, L1).
cost(C):- numCities(N), between(0,N,C).
%%%%%%% =======================================================================================
%
...
satVariable( posCost(P,Cost) ):- position(P), cost(Cost).
% Warning: more types of variables might be needed or convenient.


%%%%%%  2. Clause generation for the SAT solver:
%1 10 4 14 12 15 18 7 13 19 6 17 8 3 20 16 2 5 9 11 1 
...
writeClauses:- 
      ...
      exactlyOnePosition,
      trajectCost,
      exactlyOnePositionCost,
      atMostCost,
    true, !.
writeClauses:- told, nl, write('writeClauses failed!'), nl,nl, halt.
%

startEndCity:-  writeClause([visited(1,0)]), numCities(N), writeClause([visited(1,N)]), writeClause([posCost(0,0)]), fail.
startEndCity.

hasTraject:-  city(I1), city(I2),\+consecutiveCities(I1,I2), consecutivePositions(P1,P2), writeClause([-visited(I1,P1), -visited(I2,P2)]),fail.
hasTraject.

exactlyOneCity:- city(I), findall(visited(I,P), (position(P), P\=0),Lits), exactly(1, Lits), fail.
exactlyOneCity.

exactlyOnePosition:- position(P), findall(visited(I,P), city(I),Lits), exactly(1, Lits), fail.
exactlyOnePosition.

trajectCost:- consecutiveCities(I1,I2), consecutivePositions(P1,P2), cost(C), C < 20, X is ((I1+I2) mod 2)+C, writeClause([-visited(I1,P1),-visited(I2,P2),-posCost(P1,C),posCost(P2,X)]),fail.
trajectCost.


exactlyOnePositionCost:- position(P), findall(posCost(P,C), cost(C),Lits), exactly(1, Lits), fail.
exactlyOnePositionCost.

atMostCost:- position(P), maxCost(M), cost(Cost), Cost > M, writeClause([-posCost(P,Cost)]),fail.
%atMostCost:- numCities(N), position(N), maxCost(Max), findall(posCost(N,C), (cost(C), C > Max), Lits), exactly(0,Lits),fail.
atMostCost.
%visited(I1,P1), visited(I2,P2), consecutivePositions(P1,P2), position(P1),position(P2), fail.


%exactlyOnePositonOneCity
%atMostCost
%atMostCost:- position(P1), P2 is P1+1, position(P2), visited(C1,P1), visited(C2,P2), C1 mod 2 == C2 mod 2,

%%%%%%  3. DisplaySol: show the solution. Here M contains the literals that are true in the model:

%displaySol(M):- nl, write(M), nl, nl, fail.
%displaySol(M):- position(P), member(posCost(P,C),M), write(P-C), write(' '), nl, fail.
