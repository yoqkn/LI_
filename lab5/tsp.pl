...
%alternative:
%%%%Si la suma de distancias minimas de las ciudades que quedan + el camino hasta ahora > Kms del mejor camino encontrado -> falla
...
tsp( Cities, AccumulatedKm, [ CurrentCity | RouteSoFar ] ):-
    select( City, Cities, RemainingCities ),  % select next city to visit
   	myselect( CurrentCity, City, Cities),
    distance( CurrentCity, City, Km ),  AccumulatedKm1 is AccumulatedKm+Km.
    tsp( RemainingCities, AccumulatedKm1, [ City, CurrentCity | RouteSoFar ] ), write(RouteSoFar).
...
%%% Selecciona la ciudad que haya menor distancia con la ciudad actual
myselect( CurrentCity, City, Cities):-
    findall((Km,C), (member(C, Cities), CurrentCity \= C, distance(CurrentCity,C,Km)), Kms), minimalKmCity(Kms, X), X = (Km, C), C is City.
    
    %sort Kms de menor a major,
    %seleccionar la primera

/*
    kk(CurrentCity, X):-
    findall((Km,X), (between(1,22, X), X \= CurrentCity, distance(CurrentCity, X, Km)), Kms), write(Kms),
    sortByDistance(Kms, X),write(X).
    %sort Kms de menor a major,
    %seleccionar la primera
*/
%sortByDistance(Kms, P):- permutacion(Kms, P), isSorted(P), write(P).
minimalKmCity([X], X).
