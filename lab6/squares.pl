...
main:- 
    ...
    length(ColVars,N), % get list of N prolog vars: Row coordinates of each small square
...
nonoverlapping(N,[S|Sides],[R|RowVars],[C|ColVars]):-
	...
	N2 is N-1,
	nonoverlapping(N2, Sides, RowVars, ColVars).

noOverlap(_,_,_,[],[],[]).	
noOverlap(S1,R1,C1,[S2|Sides],[R2|RowVars],[C2|ColVars]):-
	S1+R1 #=< R2 #\/ S1+C1 #=< C2 #\/ S2+R2 #=< R1 #\/ S2+C2 #=< C1,
	noOverlap(S1,R1,C1,Sides,RowVars,ColVars).
