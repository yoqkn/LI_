...
p:-	ejemplo1(RowSums,ColSums),
	...
	declareConstraints(MatrixByRows, MatrixByCols, RowSums, ColSums, L),
...
declareConstraints(MatrixByRows, MatrixByCols, RowSums, ColSums, L):-
	L ins 0..1,
	checkSums(MatrixByRows, RowSums),
	checkSums(MatrixByCols, ColSums).
	
checkSums([],[]):-!.
checkSums([R|Matrix], [S|Sums]):- sum(R, #=, S), checkSums(Matrix, Sums).
