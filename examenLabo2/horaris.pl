...
%eachCourseAtMostOneHour
%eachRoomEachHourAtMostOneCourse
%sameYearCoursesNoOverlap
...
eachCourseExactlyOneRoom:- courseRooms(C,Rs), findall(cr(C,R),  member(R,Rs), Lits), exactly(1,Lits),fail.
eachCourseExactlyOneRoom.

eachCourseMaxOncePerDay:-  course(C), day(D), findall(cdh(C,D,H), hour(H), L), atMost(1,L), fail.
eachCourseMaxOncePerDay.

%One hour of a course day
relateVarsCDHwithCD:-  course(C), day(D), findall(cdh(C,D,H), hour(H), Lits), expressOr(cd(C,D),Lits), fail.
relateVarsCDHwithCD.

eachCourseRightNumberOfDays:- courseHours(C, Hrs), findall(cd(C,D), day(D), Lits), exactly(Hrs,Lits), fail.
eachCourseRightNumberOfDays.

noOverlapCoursesSameYear:- 
	year(Y), day(D), hour(H), 
	courseYear(C1,Y), courseYear(C2,Y), C1 \= C2, 
...
noOverlapRooms:- 
	day(D), hour(H),
	course(C1), course(C2), C1 \= C2,
	%courseRooms(C1,Rs1), courseRooms(C2, Rs2),
